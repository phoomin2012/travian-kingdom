...
(function ($, window, document) {
	...
	function onRouteChange (param, removed) {
		...
		if (param == 'tribeSelection') {
			if (!removed) {
				...
				mellon.loadServerList(setupServerSelection);
			} else {
				showMainPageScrollbar();
				$mainPage.removeClass('animate');
				if(Startpage['version']['signup'] == 3) {
		...
		if (param == 'logout' && !removed) {
			...
			$(window).one('mellon.modalClose', function () {
	...
	function setupTribeSelection () {
		...
		onClick($tribeSelection.find('.selectButton'), function () {
			if (Startpage['version']['signup'] == 3) {
				...
				}, mellonStyles['signup']['C'], mellon.getSelectedServer(), '/userParam/' + ['gaul', 'teuton', 'roman'][selectedTribeId]);
				$tribeSelection.addClass('tribeSelected');
				return;
			}
			if (mellon.getSelectedServer()) {
				Tracking.trackZarget("Startpage: Choose Tribe");
				Tracking.track('Conversion', 'PlayNow');
	...
	function setupServerSelection () {
		...
		function selectServer (serverData) {
			...
			$('.serverOption .serverName').text(serverData['applicationInstanceId']).show();
		}
		...
		for (var i = 0; i < serverList.length; i++) {
			...
			$ser.find('.serverName').text(server['applicationInstanceId'].toUpperCase()).show();
			$ser.find('.serverPopulation').text(server['population']).show();
			$ser.find('.serverUptimeDays').text(server['serverAge']).show();

			if (server['recommended'] || serverList.length == 1) {
				selectServer(server);
...
function MellonHelper () {
	 ...
	 * @param additonalUrlParam - (optional) can be used to pass some additional parameter to the mellon url e.g. the referral id
	 */
	this.triggerLink = function (urlAction, firstParam, secondParam, applicationData, additonalUrlParam) {
		var $link = $('<a>').addClass('jqFenster').data({
			mellonIframeUrl: mellonUrl[urlAction](firstParam, secondParam) + (additonalUrlParam || ''),
 ...
 * @param additonalUrlParam - (optional) can be used to pass some additional url parameter when calling the dialogue
 */
MellonHelper.appendMellonDialogue = function(parent, url, resizeCallback, stylesUrl, applicationData, additonalUrlParam) {
	if (applicationData && applicationData['applicationCountryId'] && applicationData['applicationInstanceId']) {
		mellonConfig['application']['countryId'] = applicationData['applicationCountryId'];
		mellonConfig['application']['instanceId'] = applicationData['applicationInstanceId'];
	}
	...
	var channel = (new MellonBridge(mellonConfig)).getNewChannel(url + (additonalUrlParam || ''));
...
Tracking.trackZarget = function (step) {
	window.ZargetFunnelAPI = window.ZargetFunnelAPI || [];
	window.ZargetFunnelAPI.push("zg_vpage", step);
};
...
function setSignUpVersion (version) {
	// To change the signup version requires some DOM manipulation
	$(document).ready(function() {
		...
		if (setVersion('signup', version, 0, 3) && version > 0 && version < 3) {
			var $registerNowDialogue = $('.registerNowFrame .dialogueContainer');
			$registerNowDialogue.addClass('loading fadeIn');
			MellonHelper.appendMellonDialogue($registerNowDialogue, mellonUrl.getRegistration(), function(w, h, elem) {
			...
			}, mellonStyles['signup'][['A', 'B'][version - 1]]);
		}
	});
}
//# sourceMappingURL=startpage.min.js.map